...
def test(Model, test_candidate, test_ill, entpair2f_idx, f_emb, batch_size, cuda_num, test_topk, best_result):
    ...
    if topk_list[1 - 1] > best_result[0]:
        best_result[0] = topk_list[1 - 1]
    if topk_list[10 - 1] > best_result[1]:
        best_result[1] = topk_list[10 - 1]
    print("hit @ 1: {:.5f}    hit @10 : {:.5f}    ".format(topk_list[1 - 1], topk_list[10 - 1]), end="")
    if test_topk >= 25:
        print("hit @ 25: {:.5f}    ".format(topk_list[25 - 1]), end="")
        if topk_list[25 - 1] > best_result[2]:
            best_result[2] = topk_list[25 - 1]
    if test_topk >= 50:
        print("hit @ 50: {:.5f}    ".format(topk_list[50 - 1]), end="")
        if topk_list[50 - 1] > best_result[3]:
            best_result[3] = topk_list[50 - 1]
    print("")
    # MRR
    MRR = 0
...
def train(Model, Optimizer, Criterion, Train_gene, f_emb_list, test_candidate, test_ill,
    ...
    for epoch in range(epoch_num):
        ...
        if (epoch + 1) % eval_num == 0 and epoch != 0 :
            start_time = time.time()
            test(Model, test_candidate, test_ill, entpair2f_idx, feature_emb, 2048, cuda_num, test_topk, best_result)
            print("test using time {:.3f}".format(time.time() - start_time))
    print('final exp result:')
    print("hit @ 1: {:.5f}    hit @10 : {:.5f}    hit @25 : {:.5f}    hit @50 : {:.5f}    MRR : {:.5f}".format(best_result[0],best_result[1],best_result[2],best_result[3],best_result[4]), end="")