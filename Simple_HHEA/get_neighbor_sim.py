...
def load_ent_pair_matrix(file_path):
    ...
    for i in range(matrix_dim):
        ...
        neighbor_sim_list.append(cos_sim(ent_pair_matrix_1[i,:], ent_pair_matrix_2[i, :]))
        ent_1_emb[int(ent_1_list[i]),:] = ent_pair_matrix_1[i,:]
        ent_2_emb[int(ent_2_list[i]) - ent_1_num,:] = ent_pair_matrix_2[i,:]
    ...
    return ent_pair_matrix_1, ent_pair_matrix_2,ent_1_id, ent_2_id, neighbor_sim_list
...
def main():
    ...
    #读取entity embedding
    kg_sim_matrix_1, kg_sim_matrix_2,  ent2id_kg1, ent2id_kg2, neighbor_sim_list= load_ent_pair_matrix(FILE_PATH)
    result_dict = {}

    ent_name_dict = dict()
    for line in open(FILE_PATH + 'ent_ids_1', 'r+', encoding='utf-8'):
        line = line.replace('\n','').split('\t')
        ent_name_dict[line[0]] = line[1]
    ...
    for line in open(FILE_PATH + 'ref_ent_ids', 'r+', encoding='utf-8'):
        line = line.replace('\n','').split('\t')
        ent_pairs_.append((line[0], line[1]))

    case_list = []
    count = 0
    for line in open(FILE_PATH + 'result_yago.txt', 'r', encoding='utf-8'):
        rank_num = line.replace('\n','')
        ent_pair = ent_pairs_[count]
        ent_pair_name = (ent_name_dict[ent_pair[0]], ent_name_dict[ent_pair[1]])
        case_list.append({'rank': int(rank_num), 'ent_pair': ent_pair, 'ent_pair_name': ent_pair_name, 'sim': neighbor_sim_list[count]})
        count += 1

    base = case_list
    rank = sorted(base, key=lambda x: (x['rank']))
    rank_r = sorted(base, key=lambda x: (x['rank']), reverse = True)

    import json
    with open(FILE_PATH + 'case_yago.json', 'w+', encoding = 'utf-8') as file_obj:
        json.dump(rank, file_obj)

    with open(FILE_PATH + 'case_yago_reverse.json', 'w+', encoding = 'utf-8') as file_obj:
        json.dump(rank_r, file_obj)
    ...
    for line in open(FILE_PATH + 'result_yago.txt', 'r', encoding='utf-8'):
        ...
        ent_pair = ent_pairs_[count]
        ent_pair_name = (ent_name_dict[ent_pair[0]], ent_name_dict[ent_pair[1]])
        case_list.append({'rank': int(rank_num), 'ent_pair': ent_pair, 'ent_pair_name': ent_pair_name,
                          'sim': neighbor_sim_list[count]})
        count += 1
    ...
    import json
    with open(FILE_PATH + 'case_yago_wo_0.json', 'w+', encoding='utf-8') as file_obj:
        json.dump(rank, file_obj)

    with open(FILE_PATH + 'case_yago_reverse_wo_0.json', 'w+', encoding='utf-8') as file_obj:
        json.dump(rank_r, file_obj)

    # load_ent_time_matrix(FILE_PATH)
    # load_ent_relation_matrix(FILE_PATH)
    # pass
    # load_ent_time_matrix_new(FILE_PATH)
