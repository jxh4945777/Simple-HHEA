...
parser.add_argument('--lang',  type=str, default='icews_yago')
...
class Simple_HHEA(nn.Module):
    def __init__(self, ent_pair_size, time_span, ent_dw_emb, ent_name_emb, ent_time_emb, ent_rel_emb, emb_size=64, device='cuda'):
        ...
        self.fc_name = nn.Linear(768, emb_size)
...
kg1_name_emb = np.loadtxt(file_path + 'init_bert_emb_768/ent_1_emb_64.txt')
kg2_name_emb = np.loadtxt(file_path + 'init_bert_emb_768/ent_2_emb_64.txt')
...
sample_list = [i for i in range(768)]
mask_id = random.sample(sample_list, int(768 * noise_ratio))
...
result_file = open(file_path + 'result_file.txt', 'a', encoding='utf-8')
